...
export async function getDestinations(..._args: any[]) {
  return demoDestinations;
}

export async function getFeaturedDestinations(..._args: any[]) {
  return demoDestinations.filter((item) => item.featured).slice(0, 6);
}

export async function getDestinationBySlug(slug: string, ..._args: any[]) {
