...
export const CreateFAQDTO = faqFields;
export type CreateFAQDTO = z.infer<typeof CreateFAQDTO>;

export const UpdateFAQDTO = faqFields
  .partial()
  .refine(nonEmptyUpdate, "At least one FAQ field must be provided");
...
export type FAQListQueryDTO = z.infer<typeof FAQListQueryDTO>;

const testimonialFields = z.object({
  name: z.string().trim().min(1, "Name is required").max(160),
  role: optionalText(120),
  company: optionalText(160),
  quote: z.string().trim().min(1, "Quote is required").max(3000),
...
export const CreateTestimonialDTO = testimonialFields;
export type CreateTestimonialDTO = z.infer<typeof CreateTestimonialDTO>;

export const UpdateTestimonialDTO = testimonialFields
  .partial()
  .refine(nonEmptyUpdate, "At least one testimonial field must be provided");
export type UpdateTestimonialDTO = z.infer<typeof UpdateTestimonialDTO>;

export const TestimonialListQueryDTO = z.object({
  ...listQuery,
  category: z.string().trim().optional(),
  serviceSlug: z.string().trim().optional(),
  published: optionalBoolean,
});
export type TestimonialListQueryDTO = z.infer<typeof TestimonialListQueryDTO>;
