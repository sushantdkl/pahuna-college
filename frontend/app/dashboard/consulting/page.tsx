...
export default function DashboardConsultingPage() {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setNotice("Consulting record deleted successfully.");
      await loadAll();
    } catch (deleteError) {
    ...
    <AdminReplicaFrame>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <ReplicaStatCard title="Total Services" value={summary.totalServices} subtitle="All service records" icon="CS" />
        ...
        {tab === "services" ? (
          <ReplicaDataCard title="Service records" description="Create, edit, activate, deactivate, or delete" count={serviceMeta.total}>
          ...
          <ReplicaDataCard title="Lead records" description="View, respond, update status, or delete" count={leadMeta.total}>
      ...
      {deletingService || deletingLead ? <ConfirmDialog title="Delete consulting record?" text="This record will be removed permanently." saving={saving} onCancel={() => { setDeletingService(null); setDeletingLead(null); }} onConfirm={() => void confirmDelete()} /> : null}
    </AdminReplicaFrame>
  );
}
