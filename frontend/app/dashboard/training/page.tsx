...
export default function DashboardTrainingPage() {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setNotice("Training record deleted successfully.");
      await loadAll();
    } catch (deleteError) {
    ...
    <AdminReplicaFrame>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <ReplicaStatCard title="Total Courses" value={summary.totalCourses} subtitle="All course records" icon="TC" />
        ...
        {tab === "courses" ? (
          <ReplicaDataCard title="Course records" description="Create, edit, publish, archive, activate, or delete" count={courseMeta.total}>
          ...
          <ReplicaDataCard title="Enrollment records" description="View, respond, update status, or delete" count={enrollmentMeta.total}>
      ...
      {deletingCourse || deletingEnrollment ? <ConfirmDialog title="Delete training record?" text="This record will be removed permanently." saving={saving} onCancel={() => { setDeletingCourse(null); setDeletingEnrollment(null); }} onConfirm={() => void confirmDelete()} /> : null}
    </AdminReplicaFrame>
  );
}
...
function Empty({ text }: { text: string }) { return <div className="py-14 text-center"><p className="font-semibold text-stone-800">{text}</p><p className="mt-2 text-sm text-stone-500">Matching records will appear here.</p></div>; }
