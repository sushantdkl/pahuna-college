...
export default function DashboardPartnersPage() {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setNotice("Partner application deleted successfully.");
      setDeleting(null);
  ...
  return (
    <AdminReplicaFrame>
      <div className="space-y-6">
        ...
        <ReplicaDataCard title="Application records" description="Search, review, approve, reject, add notes, or delete" count={total}>
              ...
              <tbody>{applications.map((application) => (
                <tr key={application._id} className="border-b border-stone-100 align-top last:border-0">
                  ...
                  <td className="py-4"><div className="flex min-w-[390px] flex-wrap gap-2">
                    ...
                    <button onClick={() => setDeleting(application)} className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">Delete</button>
                  </div></td>
                </tr>
              ))}</tbody>
            </table>
...
function DeleteDialog({ application, saving, onClose, onConfirm }: { application: AdminPartnerApplication; saving: boolean; onClose: () => void; onConfirm: () => void }) { return <ModalShell title="Delete partner application?" eyebrow="Permanent action" onClose={onClose}><p className="text-sm leading-6 text-stone-600">Delete <strong>{application.businessName}</strong>? This cannot be undone.</p><div className="mt-6 flex justify-end gap-3"><button onClick={onClose} disabled={saving} className="rounded-lg border border-stone-200 px-4 py-2 text-sm font-semibold">Cancel</button><button onClick={onConfirm} disabled={saving} className="rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{saving ? "Deleting..." : "Delete application"}</button></div></ModalShell>; }
