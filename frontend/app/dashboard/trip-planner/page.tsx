...
export default function DashboardTripPlannerPage() {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setError(deleteError instanceof Error ? deleteError.message : "Unable to delete itinerary");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <AdminReplicaFrame>
      <div className="space-y-6">
        ...
        <ReplicaDataCard title="Itinerary records" description="Search, review, edit, update status, or delete" count={meta.total}>
      ...
      {deleting ? <ConfirmDialog title="Delete itinerary?" text={`Delete ${deleting.title}? This cannot be undone.`} saving={saving} onCancel={() => setDeleting(null)} onConfirm={() => void confirmDelete()} /> : null}
    </AdminReplicaFrame>
  );
}
