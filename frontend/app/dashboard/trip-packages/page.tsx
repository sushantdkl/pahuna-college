...
export default function DashboardTripPackagesPage() {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setNotice("Trip package removed successfully.");
      setDeleting(null);
      await loadPackages();
    } catch (deleteError) {
      setError(deleteError instanceof Error ? deleteError.message : "Unable to remove package");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <AdminReplicaFrame>
      <div className="space-y-6">
        ...
        <ReplicaDataCard title="Package listings" description="Search, edit, publish, feature, or archive packages" count={meta.total}>
...
function ConfirmDialog({ tripPackage, saving, onCancel, onConfirm }: { tripPackage: AdminTripPackage; saving: boolean; onCancel: () => void; onConfirm: () => void }) { return <ModalShell title="Remove trip package?" eyebrow="Workspace action" onClose={onCancel}><p className="text-sm leading-6 text-stone-600">Remove <strong>{tripPackage.title}</strong>? This item will be removed from the workspace.</p><div className="mt-6 flex justify-end gap-3"><button onClick={onCancel} disabled={saving} className="rounded-lg border border-stone-200 px-4 py-2 text-sm font-semibold">Cancel</button><button onClick={onConfirm} disabled={saving} className="rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{saving ? "Removing..." : "Remove package"}</button></div></ModalShell>; }
