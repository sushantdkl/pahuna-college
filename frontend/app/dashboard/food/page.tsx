...
export default function DashboardFoodPage() {
  ...
  async function handleSave(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (mode === "create") {
        await createAdminFoodProvider(payload, imageFiles);
        setNotice("Food provider added");
      } else if (selected) {
        await updateAdminFoodProvider(selected._id, payload, imageFiles);
  ...
  return (
    <AdminReplicaFrame>
      ...
      {mode ? <FoodFormDialog mode={mode} form={form} imageFiles={imageFiles} error={formError} isSaving={isSaving} onChange={setForm} onImageFilesChange={setImageFiles} onClose={() => { setImageFiles([]); setMode(null); }} onSubmit={handleSave} /> : null}
