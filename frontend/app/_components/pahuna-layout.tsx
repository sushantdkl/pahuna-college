...
export function DashboardFrame({
  ...
  return (
    <main className="min-h-screen bg-[#f6f0e5] text-stone-950 lg:grid lg:grid-cols-[280px_1fr]">
      <aside className="border-b border-emerald-900/10 bg-[#fffdf7] px-4 py-4 lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r lg:px-5">
        <Link href="/" className="flex items-center gap-2 rounded-2xl px-2 py-2">
          <Image src="/pahuna-icon.svg" alt="Pahuna" width={36} height={36} className="h-9 w-9" />
          <span className="text-xl font-black tracking-tight text-emerald-800">PAHUNA</span>
        </Link>
        <nav className="no-scrollbar mt-4 flex gap-2 overflow-x-auto pb-1 lg:grid lg:gap-2 lg:overflow-visible lg:pb-0">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
        ...
        <div className="mt-6 hidden h-1 w-24 rounded-full bg-amber-400 lg:block" />
      </aside>
