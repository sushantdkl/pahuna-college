...
export default function ResetPasswordPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.success) {
        setError(response.message || "This reset link is invalid or expired.");
        return;
      }
      ...
      <Card className="w-full max-w-md rounded-[28px] border-emerald-900/10 bg-white/95 shadow-xl shadow-emerald-950/10">
        <CardContent className="p-8">
          ...
          <p className="mt-3 text-sm leading-6 text-stone-600">
            Choose a new password for your Pahuna account. Reset links expire
            and work only once.
          </p>

          {!token ? (
            <div className="mt-7 flex items-center gap-2 rounded-2xl bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800">
